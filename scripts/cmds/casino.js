...
module.exports = {
  ...
  onStart: async function ({ api, event, args, message, usersData }) {
    ...
    try {
      ...
      // Dynamic VIP Background Gradient
      ...
      // Header Banner Title (Clean Text - No Unicode Emoji to avoid broken boxes)
      ctx.fillStyle = "#f1c40f";
      ctx.font = "bold 24px Sans-serif";
      ctx.fillText("DI-ABLO VIP POKER CASINO", 50, 68);
      ...
      // --- USER AVATAR DRAWING (PROFILE PICTURE) ---
      try {
        const avatarUrl = `https://graph.facebook.com/${senderID}/picture?height=500&width=500&access_token=6628568379%7Cc15a440756e44ac5b2aa361a52f5a94f`;
        const avatarImg = await loadImage(avatarUrl);
        
        ctx.save();
        ctx.beginPath();
        ctx.arc(100, 190, 45, 0, Math.PI * 2, true);
        ctx.closePath();
        ctx.clip();
        ctx.drawImage(avatarImg, 55, 145, 90, 90);
        ctx.restore();

        // Border around profile avatar
        ctx.strokeStyle = isWin ? "#2ecc71" : "#e74c3c";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(100, 190, 46, 0, Math.PI * 2, true);
        ctx.stroke();
      } catch (e) {
      ...
      ctx.fillRect(160, 115, 590, 150);
      ctx.strokeStyle = isWin ? "rgba(46, 204, 113, 0.5)" : "rgba(231, 76, 60, 0.5)";
      ctx.lineWidth = 2;
      ctx.strokeRect(160, 115, 590, 150);

      // Outcome Title (Clean Text)
      ctx.textAlign = "center";
      ctx.fillStyle = isWin ? "#2ecc71" : "#e74c3c";
      ctx.font = "bold 28px Sans-serif";
      ctx.fillText(isWin ? "[ ROYAL FLUSH • HIGH STAKES WIN ]" : "[ HOUSE TOOK THE CHIPS ]", 455, 160);

      // Win / Loss Amount
      ctx.font = "bold 36px Sans-serif";
      ctx.fillStyle = isWin ? "#4ade80" : "#f87171";
      ctx.fillText(isWin ? `+ $${winAmount.toLocaleString()}` : `- $${bet.toLocaleString()}`, 455, 220);
      ctx.textAlign = "left";
      ...
      ctx.fillText(`PLAYER: ${userName} | VIP HIGH ROLLER TABLE`, 50, 420);
